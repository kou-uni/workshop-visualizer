...
- interpretations.spark: 技術/アーキテクチャ視点の読みを **2〜3点**（各 read は1〜2文で具体的に / evidence＝実引用 必須）
- interpretations.minta: 要件/価値/UX視点の読みを **2〜3点**（各 read は1〜2文で具体的に / evidence＝実引用 必須）
重要：evidence は必ず入力からの実際の引用にすること（捏造しない）。読みは具体的に踏み込み、当たり障りのない一般論にしない。フレンドリーで端的に。`;
